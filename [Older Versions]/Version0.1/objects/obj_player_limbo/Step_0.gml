switch(state){
	...
	case "attack one":
		...
		if(scr_animation_hit_frame(0)) scr_create_hitbox(x, y, id, spr_skeleton_attack_one_damage, 3, 4, 5, image_xscale);
		
		if(input.attack && scr_animation_hit_frame_range(2, 4)){
			state = "attack two";
		}
		if(scr_animation_end()) state = "move";
	break;
	...
	case "attack two":
		...
		if(scr_animation_hit_frame(1)) scr_create_hitbox(x, y, id, spr_skeleton_attack_two_damage, 3, 4, 6, image_xscale);
	...
	case "attack three":
		...
		if(scr_animation_hit_frame(2)) scr_create_hitbox(x, y, id, spr_skeleton_attack_three_damage, 5, 4, 10, image_xscale);
