...
instance_create_layer(_player_start_x, _player_start_y, "Player_AI", obj_player_native);
...
for(var _y =1; _y < height_ -1; _y++){
	for( var _x = 1; _x < width_ -1; _x++){
		if(grid_[# _x, _y] != FLOOR){
			...
			// Add enemies
			if (point_distance(object_x, object_y, obj_player_native.x, obj_player_native.y) > global.distance_enemy_from_player){
